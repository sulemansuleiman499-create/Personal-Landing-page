# Calculator App

A simple, responsive calculator built with **HTML5**, **CSS3**, and basic JavaScript (if added).

## Live Demo
🌐 [Try the Calculator Here](https://sulemansuleiman499-create.github.io/Personal-Landing-page/)

## Screenshots

**Desktop view (laptop):**
<image-card alt="Desktop View" src="Screenshot2026-03-04-145133.png" ></image-card>

**Mobile view 1:**
<image-card alt="mobile view 1" src="mobileview1.jpg" ></image-card>   <!-- or mobileview-1.jpg or whatever exact name -->

**Mobile view 2:**
<image-card alt="mobile view 2" src="mobileview2.jpg" ></image-card>   <!-- same for second one -->

## Features
- Basic arithmetic operations (+, −, ×, ÷)
- Clear (C), Delete (DEL), Percentage (%), Decimal (.)
- Grid layout for buttons
- Mobile-friendly design

## Technologies
- HTML5
- CSS3 (Flexbox/Grid for layout)
- JavaScript (for calculations – if implemented)

Built by Suleiman – aspiring web developer from Lagos, Nigeria.  
Open to freelance HTML/CSS/JS gigs! Check my Fiverr (link coming soon).

More projects coming...
